...
parser.add_argument('-nms_tolerance', type=float, default=0.45,
...
def get_coords(json_file, filename):
    ...
    for i in data:
        ...
        ymin = i['relative_coordinates']['center_y'] - \
            i['relative_coordinates']['height'] / 2
...
def non_max_suppression(detections_, tolerance=args.nms_tolerance):
    ...
    for i in range(len(output_labels)):
        for j in range(i+1, len(output_labels)):
            if output_labels[i] == output_labels[j]:
                ...
                areai = (picked_boxes[i][2]-picked_boxes[i][0]) * \
                    (picked_boxes[i][3]-picked_boxes[i][1])
                areaj = (picked_boxes[j][2]-picked_boxes[j][0]) * \
                    (picked_boxes[j][3]-picked_boxes[j][1])
                #print(areai, areaj)
    ...
    picked_boxes = [picked_boxes[i]
                    for i in range(len(picked_boxes)) if mask[i]]
    picked_score = [picked_score[i]
                    for i in range(len(picked_score)) if mask[i]]
    output_labels = [output_labels[i]
                     for i in range(len(output_labels)) if mask[i]]
...
def save_image_pred(image_name, picked_boxes, picked_score, output_labels):
    ...
    for (start_x, start_y, end_x, end_y), confidence, label in zip(picked_boxes, picked_score, output_labels):
        (w, h), baseline = cv2.getTextSize(
            f'{label}: {round(confidence*100)}%', font, font_scale, thickness=1)
...
for filename in os.listdir(img_dir):
    ...
    os.system(f'./darknet detector test data/obj.data cfg/yolov4-obj.cfg\
                cardamage_final.weights {img_dir}/{filename} -thresh 0.2 -out results.json\
    ...
    shutil.move('predicted.jpg',
                f'./predictions/{filename.split(".")[0]}_predictions.jpg')
